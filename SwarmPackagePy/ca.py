...
class ca(intelligence.sw):
            ...
            for i in range(n):
                if flag[i] == 0:
                    ...
                    if copycat.all() == np.array([copycat[0] for _ in range(sm)]).all():
                        ...
                        P = np.array([abs(function(j) - fb) / (fmax - fmin) for j in copycat])
                    self.__agents[i] = copycat[P.argmax()]
                else:
                    ww = w + (iteration - t) / (2 * iteration)
                    cc = c - (iteration - t) / (2 * iteration)
                    r = random()
                    velocity[i] = ww * np.array(velocity[i]) + r * cc * (np.array(Pbest) - np.array(self.__agents[i]))
    ...
    @staticmethod
    def __get_inf(i, velocity, cat, csi):
        if i == 0:
